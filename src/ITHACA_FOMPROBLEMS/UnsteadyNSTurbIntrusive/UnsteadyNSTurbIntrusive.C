...
void UnsteadyNSTurbIntrusive::project(fileName folder, label nModes)
{
    ...
    if (ITHACAutilities::check_folder("./ITHACAoutput/Matrices/"))
    {
        ...
        if (!ITHACAstream::importNpy(convTensor, cStr))
        {
...
Eigen::Tensor<double, 3> UnsteadyNSTurbIntrusive::convectiveTerm(label nModes)
{
    ...
    // Export the tensor
    ITHACAstream::exportToFile(convTensor,
                                  "c_" + name(nModes) + "_t");
    return convTensor;
}
...
Eigen::MatrixXd UnsteadyNSTurbIntrusive::pressureGradientTerm(label nModes)
{
    ...
    ITHACAstream::exportToFile(kMatrix,
                                  "k_" + name(nModes));
    return kMatrix;
}
...
Eigen::MatrixXd UnsteadyNSTurbIntrusive::laplacianPressure(label nPModes)
{
    ...
    ITHACAstream::exportToFile(dMatrix, "D_" + name(nPModes));
    return dMatrix;
}
        ...
        label nPModes)
{
    ...
    ITHACAstream::exportToFile(BC1_matrix, "BC1_" + name(nUModes) + "_" + name(nPModes));
    return BC1_matrix;
}
        ...
        label nPModes)
{
    ...
    ITHACAstream::exportToFile(BC3_matrix, "BC3_" + name(nUModes) + "_" + name(nPModes));
    return BC3_matrix;
}
        ...
        label nPModes)
{
    ...
    ITHACAstream::exportToFile(BC4_matrix, "BC4_" + name(nUModes) + "_" + name(nPModes));
    return BC4_matrix;
}
