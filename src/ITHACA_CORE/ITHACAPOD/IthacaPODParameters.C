...
//#include "IthacaFVNonPolynomPhysFct.H"
            ...
            myfield_name(myfield_name)
{
  ...
  IthacaPODParameters::~IthacaPODParameters()
  {
    //todo review all ownership of this class, there is mutch to do
    delete runTimeData;
    delete mesh;
    //ref delete submesh;
    //ref to a sigleton delete ithacaLibraryParameters;
    delete ITHACAdict;
    delete template_field_U;
    delete template_field_p;
    delete template_field_nut;
    delete template_field_omega;
    delete template_field_k;
    delete nu;
    delete volume;
    delete delta;
    delete magicDelta;
    delete meanU;
    delete meanVectorDEIM;
    delete meanScalarDEIM;
    delete meanVectorDEIMMagic;
    delete meanScalarDEIMMagic;
  }

  const Foam::Vector<double>& IthacaPODParameters::get_F_0() const
  {
  ...
  void IthacaPODParameters::compute_F_mask()
  {
  ...
  template<typename T>
  void IthacaPODParameters::read_snapshot(T& snapshot, const label& i_snap, word path, word name) const
  {
  ...
  template void IthacaPODParameters::read_snapshot(volScalarField& snapshot, const label& i_snap, word path, word name) const;
  template void IthacaPODParameters::read_snapshot(volVectorField& snapshot, const label& i_snap, word path, word name) const;
  template void IthacaPODParameters::read_snapshot(volTensorField& snapshot, const label& i_snap, word path, word name) const;

  word IthacaPODParameters::get_pathHilbertSpace(word fieldName)
  {
    return get_pathHilbertSpace_fromHS(hilbertSpacePOD[fieldName]);
  }
  
  word IthacaPODParameters::get_pathHilbertSpace_fromHS(word hilbertSp)
  {
